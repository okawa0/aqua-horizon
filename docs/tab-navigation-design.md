...
- `<details name="faq">` で排他開閉（ネイティブ機能、**JS 0行**）
- 開閉アニメーションは `@supports selector(::details-content)` で囲み、
  `interpolate-size: allow-keywords` + `::details-content` で高さを補間。
  非対応環境では即時開閉（アニメーションなし）に自然にフォールバックするため、JSでの高さ計算は行わない
- 実装後にChromeで動作確認済み（排他開閉・`::details-content` / `interpolate-size` ともに対応）
